...
#define VOUT_TARGET ((uint16_t) 5300) // This is the user VOUT voltage target in mV that DC/DC converter will apply on V output
/************************************************************************************************************************/
#define BUCK_PWM_PERIOD 46080 /* 50kHz */
...
/***************************** DO NOT MODIFY ********************************/
#define MIN_DUTY 1152 // % MinDuty for Buck
#define MAX_DUTY 20736 // % MaxDuty for Buck

#define SAT_LIMIT 20736 // PI Integral term saturation value
...
volatile int32_t seterr, pid_out;
...
int main(void)
{
  ...
  while (1)
  {
//	    Vout = (HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_1) * 3300)/0x1000;
...
static void MX_ADC1_Init(void)
{
  ...
  */
  ...
  hadc1.Init.ExternalTrigConv = ADC_EXTERNALTRIGCONVHRTIM_TRG3;
...
static void MX_HRTIM1_Init(void)
{
  ...
  pCompareCfg.CompareValue = 20736;
  ...
  pCompareCfg.CompareValue = 20736+23040;
...
int32_t my_PID_Controller(void)
{
  ...
  last_error = error;
  error = (int32_t) VoutT - (int32_t ) VoutConversion;
  errorSum = errorSum + error * 5;
